  ...
  return (
    <div className="flex min-h-[400px] flex-col items-center justify-center gap-4 text-center">
      ...
      <div className="space-y-2">
        <h3 className="text-lg font-semibold">{hasFilters ? 'No returns found' : 'No purchase returns yet'}</h3>
        <p className="text-sm text-muted-foreground">
          {hasFilters ? 'Try adjusting your filters to see more results.' : 'Purchase returns will appear here once you create them.'}
        </p>
      </div>
...
export function PurchaseReturnsTable({ refreshKey }: { refreshKey?: number }) {
  const { format: formatCurrency } = useCurrency()
  const [filters, setFilters] = useState<PurchaseReturnsFilters>(DEFAULT_RETURN_FILTERS)
    ...
    <>
      <Card>
        <CardContent className="p-6">
          ...
          {isLoading ? <LoadingState /> : purchaseReturns.length === 0 ? <EmptyState hasFilters={hasFilters} onClearFilters={handleClearFilters} /> : (
            <>
                  ...
                  <TableBody>
                    {purchaseReturns.map((returnItem) => (
                      <TableRow key={returnItem.id}>
                        ...
                        <TableCell><span className="font-mono text-sm">{returnItem.purchase?.invoiceNumber || '-'}</span></TableCell>
                        <TableCell>{returnItem.purchase?.party?.name || '-'}</TableCell>
              ...
              <div className="mt-4 flex items-center justify-between">
                <div className="text-sm text-muted-foreground">
                  Showing {purchaseReturns.length > 0 ? (currentPage - 1) * perPage + 1 : 0} to {purchaseReturns.length > 0 ? Math.min((currentPage - 1) * perPage + purchaseReturns.length, totalItems || purchaseReturns.length) : 0} of {totalItems || purchaseReturns.length} entries
                </div>
                <div className="flex items-center gap-2">
                  <Button
                    variant="outline"
      ...
      <PurchaseReturnDetailsDialog purchaseReturn={selectedReturn} open={isDetailsOpen} onOpenChange={setIsDetailsOpen} />
    </>
  )
}
