...
import type { PurchasesReportData } from '@/types/api.types'

const CACHE_PREFIX = 'cache:reports:purchases'
...
export function usePurchasesReport(params: TransactionReportParams) {
  const { isOnline } = useOnlineStatus()

  const cacheKey = useMemo(() => buildCacheKey(params), [params])

  const query = useQuery<PurchasesReportData>({
    queryKey: ['reports', 'purchases', params],
    queryFn: async () => {
      if (isOnline) {
        const response = await reportsService.getPurchasesReport(params)
        setCache(cacheKey, response.data, { ttl: REPORTS_TTL_MS })
        return response.data
      }

      const cached = getCache<PurchasesReportData>(cacheKey, { ttl: REPORTS_TTL_MS })
  ...
  useEffect(() => {
    ...
    toast.error(appError.message || 'Failed to load purchases report')
