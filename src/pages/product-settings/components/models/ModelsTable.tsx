...
export function ModelsTable({ searchQuery, refreshTrigger, onEdit }: ModelsTableProps) {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      let result
      if (searchQuery) {
        result = await modelsService.filter({
          search: searchQuery,
          page: currentPage,
          per_page: perPage,
        })
      ...
      const normalizePaginated = <T,>(resp: unknown, pageSize: number) => {
        ...
        if ('data' in (resp as Record<string, unknown>)) {
          ...
          if (d && typeof d === 'object' && 'data' in (d as Record<string, unknown>)) {
            ...
            const lastPage =
              typeof lastVal === 'number' ? lastVal : Number(lastVal) || Math.ceil(total / pageSize)
      ...
      } = normalizePaginated<ProductModel>(result, perPage)
  ...
  const handleStatusToggle = async (model: ProductModel) => {
    try {
      ...
      // Optimistic update
      setData((prev) =>
        prev.map((item) => (item.id === model.id ? { ...item, status: newStatus ? 1 : 0 } : item))
      )
