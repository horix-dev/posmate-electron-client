...
export function BrandsTable({ searchQuery, refreshTrigger, onEdit }: BrandsTableProps) {
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            if (searchQuery) {
                const response = await brandsService.filter({ search: searchQuery })
                            ...
                            data.map((brand) => (
                                <TableRow key={brand.id}>
                                    ...
                                    <TableCell className="font-medium">
                                        <div className="flex items-center gap-2">
                                            {brand.icon ? (
                                                <img src={brand.icon} alt="" className="h-8 w-8 rounded object-cover" />
                                            ) : null}
                                            {brand.brandName}
                                        </div>
                                    </TableCell>
                                    ...
                                    <TableCell className="text-right">
                                        <div className="flex items-center justify-end gap-2">
                                            ...
                                            <Button variant="ghost" size="icon" onClick={() => handleStatusToggle(brand)}>
                                                <Power className={`h-4 w-4 ${brand.status === 1 ? 'text-green-500' : 'text-red-500'}`} />
                                            </Button>
