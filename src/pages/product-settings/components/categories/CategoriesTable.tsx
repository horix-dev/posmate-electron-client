...
export function CategoriesTable({ searchQuery, refreshTrigger, onEdit }: CategoriesTableProps) {
    ...
    const fetchData = useCallback(async () => {
        setIsLoading(true)
        try {
            // Note: If searchQuery is present, the API might not support per_page depending on implementation,
            // but we'll send it anyway. The generic filter might return all, so we might need client-side pagination for search?
            // Assuming server-side for now based on service definition.

            let result
            if (searchQuery) {
                result = await categoriesService.filter({ search: searchQuery, page: currentPage, per_page: perPage })
            } else {
                result = await categoriesService.getAll({ page: currentPage, per_page: perPage })
            }
            ...
            const { items, total: responseTotal, lastPage: responseLastPage } = normalizePaginated<Category>(result, perPage)
    ...
    const handleStatusToggle = async (category: Category) => {
        try {
            ...
            // Optimistic update
            setData(prev => prev.map(c => c.id === category.id ? { ...c, status: newStatus ? 1 : 0 } : c))
                                ...
                                <TableRow key={category.id}>
                                    ...
                                    <TableCell className="text-right">
                                        <div className="flex items-center justify-end gap-2">
                                            ...
                                            <Button variant="ghost" size="icon" className="text-destructive" onClick={() => handleDeleteClick(category.id, category.categoryName)}>
