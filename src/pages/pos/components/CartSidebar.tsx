...
const CartTotalsSection = memo(function CartTotalsSection({
  ...
  const handleClearDiscount = useCallback((e: React.MouseEvent) => {
    e.stopPropagation()
      ...
      {isEditingDiscount ? (
        ...
        >
          <div className="flex flex-col">
            <span className={`text-sm font-medium transition-colors ${discountValue > 0 ? 'text-green-600 dark:text-green-400' : 'text-muted-foreground'}`}>
              Discount
            </span>
            {discountValue > 0 && (
          ...
          <div className="flex items-center gap-2">
            <span className={`text-right text-sm font-bold transition-colors ${discountValue > 0 ? 'text-green-600 dark:text-green-400' : 'text-muted-foreground'}`}>
              {totals.discountAmount > 0 ? `-${formatCurrency(totals.discountAmount)}` : 'No discount'}
            </span>
            {discountValue > 0 && (
              <Button
                variant="ghost"
