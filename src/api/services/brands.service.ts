...
export const brandsService = {
     ...
     */
    getAll: async (params?: { page?: number; per_page?: number }): Promise<PaginatedApiResponse<Brand[]>> => {
        const { data } = await api.get<PaginatedApiResponse<Brand[]>>(API_ENDPOINTS.BRANDS.LIST, { params })
        return data
    },
    ...
    updateStatus: async (id: number, status: boolean): Promise<ApiResponse<Brand>> => {
        const { data } = await api.patch<ApiResponse<Brand>>(`${API_ENDPOINTS.BRANDS.UPDATE(id)}/status`, { status })
        return data
    },
    ...
    deleteMultiple: async (ids: number[]): Promise<ApiResponse<null>> => {
        const { data } = await api.post<ApiResponse<null>>(`${API_ENDPOINTS.BRANDS.LIST}/delete-all`, { ids })
        return data
    },
}
