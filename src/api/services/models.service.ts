...
export const modelsService = {
  ...
  getAll: async (params?: {
    page?: number
    per_page?: number
  }): Promise<PaginatedApiResponse<ProductModel[]>> => {
    const { data } = await api.get<PaginatedApiResponse<ProductModel[]>>(
      API_ENDPOINTS.PRODUCT_MODELS.LIST,
      { params }
    )
    return data
  },
  ...
  filter: async (params: {
    ...
    const { data } = await api.get<PaginatedApiResponse<ProductModel[]>>(
      `${API_ENDPOINTS.PRODUCT_MODELS.LIST}/filter`,
      { params }
    )
    return data
  },
  ...
  getById: async (id: number): Promise<ApiResponse<ProductModel>> => {
    const { data } = await api.get<ApiResponse<ProductModel>>(
  ...
  updateStatus: async (
    ...
    const { data } = await api.patch<ApiResponse<ProductModel>>(
      API_ENDPOINTS.PRODUCT_MODELS.UPDATE(id),
      payload
    )
    return data
  ...
  deleteMultiple: async (ids: number[]): Promise<ApiResponse<null>> => {
    const { data } = await api.post<ApiResponse<null>>(
      `${API_ENDPOINTS.PRODUCT_MODELS.LIST}/delete-all`,
      { ids }
    )
    return data
  },
