...
**Highlights**:
1. **Warehouse-Aware Batch Rows**
   - Each batch row now collects warehouse, quantity, purchase/sale/wholesale/dealer prices, profit %, and manufacturing/expiry dates.
   - Warehouses load from the API (with cache fallback) so users can only submit valid locations.
2. **Flat Array Serialization**
   - Form submission emits `batch_no[0]`, `productStock[0]`, `warehouse_id[0]`, etc., and sets `inventory_tracking_mode=batch` + `product_type=variant` whenever batch rows exist.
   - Numeric validation tightened for all per-row price/profit fields so empty strings or junk data are rejected early.
3. **Quality of Life**
   - Batch UI surfaces warnings when no warehouse is configured and keeps auto-generated lot numbers intact.
...
## 2026-02-12 — Batch Profit Field Reverted

**Change**: Removed the per-batch “Profit %” input and related payload plumbing from the product create form until backend support is finalized.
...
## 2026-02-12 — Auto Batch Number Generation

**Feature**: Batch/Lot identifiers are now generated by the form and locked so users cannot alter them manually.

**Highlights**:
1. **Deterministic IDs**
   - Each added batch row now receives a `LOT-YYMMDD-XXXXXX` style number composed of the current date and a random hash segment.
   - Numbers are injected automatically when batch tracking toggles on or when merchants add extra rows.
2. **Read-Only UI**
   - Batch number inputs are rendered read-only/disabled with helper text so operators know the field cannot be edited.
   - Existing products keep their stored lot numbers; missing values are auto-populated on load to avoid submission errors.
3. **Better Form Flow**
   - Batch Entries card now appears immediately after the Classification block, keeping all product metadata together before pricing inputs.
4. **Schema Barrel Fix**
   - Re-exported `BatchFormValue` from the schemas index so components using the barrel can access the new type without direct file imports.
5. **Backend Compatibility**
   - Until the Laravel endpoint supports multiple initial lots, the form now submits only the first batch and sends scalar fields (`batch_no`, `mfg_date`, etc.) to avoid MySQL's "Array to string conversion" error.
   - Add/remove controls are hidden to prevent users from entering extra batches that would be ignored by the API.

**Files Modified/Created**:
- `src/pages/products/ProductFormPage.tsx`
- `src/pages/products/schemas/product.schema.ts`
- `src/pages/products/schemas/index.ts`
- `DEVELOPMENT_LOG.md`

## 2026-02-12 — Batch Entry UI For Product Creation

**Feature**: Allow merchants to capture initial batch/lot rows directly in the Add Product form when batch tracking is enabled.

**Highlights**:
1. **Product Form UX**
   - Batch checkbox now reveals a "Batch Entries" card with dynamic rows (add/remove) covering lot number, quantity, pricing, and optional manufacture/expiry dates.
   - Initial stock field hides automatically for batch-tracked products to prevent double-counting and shows guidance text.
2. **Validation & Submission**
   - Frontend validation enforces at least one batch row with quantity before saving batch-tracked products.
   - FormData builder now emits `batch_no[]`, `productStock[]`, pricing, and date arrays expected by the backend; traditional `productStock` is skipped for batch items.
3. **Edit Support**
   - Existing batch products hydrate the new UI using stock records so users can review their lots when opening the form.
